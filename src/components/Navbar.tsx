      ...
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-16">
          ...
          >
            <div className="space-y-2">
              <span className={`block w-8 h-0.5 bg-gray-600 transform transition-transform ${isOpen ? 'rotate-45 translate-y-2.5' : ''}`}></span>
        ...
        <div className={`md:hidden transition-all duration-300 ease-in-out ${isOpen ? 'max-h-64 opacity-100' : 'max-h-0 opacity-0'} overflow-hidden`}>
          <div className="pb-3 space-y-2">
            <MobileNavLink to="/shirts" onClick={() => setIsOpen(false)}>Shirts</MobileNavLink>
            <MobileNavLink to="/tshirts" onClick={() => setIsOpen(false)}>T-Shirts</MobileNavLink>
            <MobileNavLink to="/pants" onClick={() => setIsOpen(false)}>Pants</MobileNavLink>
